...
def preprocess_titles(df):
    ...
    def preprocess_and_classify_title(title):
        if title.startswith("Marvel's"):
            title = title.replace("Marvel's", "")

        main_title = title.split(":")[0].strip()
        content_type = (
            "TV Show"
            if "Season" in title or "Episode" in title or ":" in title
            else "Movie"
        )
...
def clean_and_format_data(df):
    ...
    df = df.drop(
        columns=["Attributes", "Supplemental Video Type", "Bookmark", "Latest Bookmark"]
    )
...
def categorize_devices(df):
    ...
    def categorize_device(device_name):
        ...
        if "iphone" in device_name or "ipad" in device_name:
            ...
            "chrome" in device_name or "edge" in device_name or "safari" in device_name
        ):
            return "Web Browsers"
        elif (
            "android tv" in device_name
            or "smart tv" in device_name
            or "tv" in device_name
        ):
            return "Smart TVs"
        elif (
            "streaming stick" in device_name
            or "chromecast" in device_name
            or "firetv" in device_name
            or "apple tv" in device_name
            or "roku" in device_name
        ):
...
# ------------------------------
DATA_PATH = (
    "/Users/pranavsukumaran/Desktop/Personal_dev/netflix/data/ViewingActivity.csv"
)
...
output_dir = "/Users/pranavsukumaran/Desktop/Personal_dev/netflix/project/data/"
